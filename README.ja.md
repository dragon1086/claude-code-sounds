...
| 選択肢 | 動作 |
|--------|------|
| **user (global)** ✅ | すべてのプロジェクトで自動的に音が鳴る |
| project | 音が鳴らない — プロジェクトごとに `setup-project` を1回実行する必要あり（下記参照） |
| local | project と同じだが git 除外（個人設定用）— `setup-project` が必要 |

> **インストール後：** フックを有効にするため Claude Code を再起動してください。

#### project スコープの修正方法

project スコープを選択した場合、プロジェクト内で1回実行：

```bash
bash "$(find ~/.claude/plugins/cache/claude-code-sounds -name "claude-sounds.sh" | head -1)" setup-project
```

Claude Code を再起動すると音が鳴ります。
